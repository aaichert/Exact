...
For Ubuntu 18.4 with old versions of docker-compose you can download a more up-to-date one
```
...
e.g. Ubuntu
```
sudo apt-get install wget unzip and openssl
```

Download the latest source
```
wget https://github.com/ChristianMarzahl/Exact/archive/master.zip
unzip master.zip
rm master.zip
mv Exact_master exact
cd exact
```

Rename the files `env.dev` and `env.dev.db` to `env.prod` and `env.prod.db` and change settings according to your preferences.
```
mv env.dev env.prod
mv env.dev.db env.prod.db
```
...
Install docker and docker-compose, e.g. for Ubuntu
```
sudo apt-get install docker.io docker-compose
```

For old versions of Ubuntu (e.g. 18.4) with old versions of docker-compose you should download a more up-to-date one, for example
```
sudo apt-get purge docker-compose
sudo wget "https://github.com/docker/compose/releases/download/1.24.0/docker-compose-$(uname -s)-$(uname -m)" -O /usr/bin/docker-compose
sudo chmod +x /usr/bin/docker-compose
...
Install wget unzip and openssl
e.g. Ubuntu
```
sudo apt-get install wget unzip openssl
```
...
Generate some random secret, e.g.
```
openssl rand -base64 42
```
Finally, edit settings.py according to your needs.
```
nano ./exact/exact/settings.py
```

The SECRET_KEY should contain the random secret you have just created. In addition, ALLOWED_HOSTS has to be updated to contain the IP or domain name of the machine if you want to be able to access Exact from the outside.
...
---
Hint: If you would like to adjust the port on which the server is running, you can do this here:
